document.addEventListener("DOMContentLoaded", () => {
  ...
  function openSidebar(){ body.classList.add('sidebar-open'); if (backdrop) backdrop.style.display = 'block'; }
  function closeSidebar(){ body.classList.remove('sidebar-open'); if (backdrop) backdrop.style.display = 'none'; }
  ...
  window.addEventListener('resize', () => {
    if (window.innerWidth < 768){
      closeSidebar(); sidebar.classList.remove('collapsed'); body.classList.remove('sidebar-collapsed');
    } else { closeSidebar(); }
  });
  window.addEventListener('resize', () => {
    if (window.innerWidth > 768){
      closeSidebar(); sidebar.classList.add('collapsed'); body.classList.add('sidebar-collapsed');
    } else { closeSidebar(); }
  });
  links.forEach(link => {
    link.addEventListener('click', (e) => {
      if (window.innerWidth >= 768 && sidebar.classList.contains('collapsed')){
        e.preventDefault(); sidebar.classList.remove('collapsed'); body.classList.remove('sidebar-collapsed');
      }
  ...
  async function fetchDonors(){
    const res = await fetch(`${API_BASE_URL}/user_api.php?action=list&role=donor&status=approved&t=${Date.now()}`, {
  ...
  function renderDonors(donors, donations){
    ...
    const rows = donors.map(u => {
      const name = (u.organization_name && u.organization_name.trim()) ? u.organization_name.trim() : (u.name || '').trim();
  ...
  async function init(){
    try {
      ...
      const tbody = document.querySelector('main .table tbody');
      if (tbody) tbody.innerHTML = `<tr><td colspan="7" class="text-center py-3">Loading donors...</td></tr>`;
      ...
      const tbody = document.querySelector('main .table tbody');
      if (tbody) tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger">Failed to load donors (${escapeHtml(err.message)})</td></tr>`;
    }
  }
});