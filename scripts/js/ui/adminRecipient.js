...
function showImportModal(title, html){
  ...
  if (!body || !label || !el){
    console.warn('Import result modal elements not found, falling back to alert');
...
(function(){
  ...
  function fillDown(rows, keys){
    ...
    rows.forEach(r => {
      ...
      keys.forEach(k => {
        ...
        if (v) { last[k] = nr[k]; }
        else if (last[k] != null) { nr[k] = last[k]; }
      });
      out.push(nr);
    });
    return out;
  ...
  async function showPreviewAndMaybeImport(rawRows){
    ...
    if (!modalEl || !tableBody) { console.warn('Preview modal elements missing'); return; }
    ...
    function showPreviewWarning(message){
      ...
      if (!warn){
        ...
        const actions = body.querySelector('.d-flex.justify-content-between.align-items-center');
    ...
    function propagateOrgEmails(){
      ...
      const normalizeOrg = (s)=> String(s||'').trim().toLowerCase();
      ...
      if (uniqueOrgs.size === 1){
        ...
        if (!hasAnyEmail){
          ...
          showPreviewWarning('Info: No email found for this organization. A placeholder email has been generated for preview and will be used on import unless you change it.');
        } else {
          showPreviewWarning('');
        }
      } else {
    ...
    if (master){ master.checked = true; master.addEventListener('change', ()=>{
      tableBody.querySelectorAll('input.row-check').forEach(cb => { cb.checked = master.checked; });
      updateStatus();
    }); }
    if (selAll) selAll.onclick = ()=>{ tableBody.querySelectorAll('input.row-check').forEach(cb => cb.checked = true); if (master) master.checked = true; updateStatus(); };
    if (deselAll) deselAll.onclick = ()=>{ tableBody.querySelectorAll('input.row-check').forEach(cb => cb.checked = false); if (master) master.checked = false; updateStatus(); };
    if (validateBtn) validateBtn.onclick = ()=>{ tableBody.querySelectorAll('tr').forEach(tr => validateRow(tr)); updateStatus(); };
    if (fillDownCheck) fillDownCheck.onchange = ()=>{
      norm = fillDownCheck.checked ? fillDown(originalNorm, fillDownKeys) : originalNorm.slice();
      renderPreviewTable(norm);
      // re-wire events after re-render
      tableBody.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.addEventListener('change', updateStatus));
      tableBody.querySelectorAll('input[type="text"]').forEach(inp => inp.addEventListener('input', (e)=>{ const tr = e.target.closest('tr'); validateRow(tr); updateStatus(); }));
    ...
    async function doImport(){
      ...
      tableBody.querySelectorAll('tr').forEach(tr => {
        ...
        if (!(String(obj['name of beneficiary']||obj['organization_name']||obj['recipient_name']||'').trim() || String(obj['name']||'').trim())) return;
        payload.push(obj);
      });
      ...
      let j;
      try{
        const res = await fetch(`${API_BASE_URL}/users/index.php?action=importRecipients`, {
      ...
      if (summary.errors && summary.errors.length){
        ...
        const list = summary.errors.slice(0, 10).map(e => `<li><code>Row ${e.row}</code>: ${escapeHtml(e.error || '')}</li>`).join('');
        const html = `<div class="mb-2">Import completed.</div>
          <div class="mb-2"><span class="badge bg-success me-2">Inserted: ${summary.inserted || 0}</span>
          <span class="badge bg-danger">Errors: ${summary.errors.length}</span></div>
        ...
        const html = `
          ...
          <div><span class="badge bg-success">Inserted: ${summary.inserted || 0}</span></div>`;
    ...
    if (importBtn){ importBtn.onclick = async ()=>{
      ...
      let invalidSelected = 0; let selected = 0;
      tableBody.querySelectorAll('tr').forEach(tr => {
        const checked = tr.querySelector('input.row-check')?.checked;
        if (!checked) return;
        selected++;
      ...
      if (!selected){ showImportModal('Import Error', '<div class="text-danger">Please select at least one row.</div>'); return; }
      if (invalidSelected){ showImportModal('Import Error', `<div class="text-danger">${invalidSelected} selected row(s) have errors. Please fix highlighted rows.</div>`); return; }
      try { await doImport(); } catch(err){ console.error(err); showImportModal('Import Error', `<div class="text-danger">${escapeHtml(err.message||'Import failed')}</div>`);} 
    }; }
    ...
    function updateStatus(){
      ...
      const selected = tableBody.querySelectorAll('input.row-check:checked').length;
  ...
  async function fetchRecipients(){
    const res = await fetch(`${API_BASE_URL}/users/index.php?action=list&role=recipient&status=approved&t=${Date.now()}`, {
    ...
    const rows = items.map(u => {
      ...
      return `
        <tr>
          ...
          <td class="d-none d-md-table-cell" data-label="Contact#">${escapeHtml(contactNo || '—')}</td>
          <td class="d-none d-lg-table-cell text-break" data-label="Email Address">${escapeHtml(email || '—')}</td>
          <td data-label="Status">${status}</td>
          <td data-label="Actions"><a href="#" data-user-id="${u.user_id}">View / Edit</a></td>
        </tr>
      `;
    });
  ...
  async function init(){
    try{
      const tbody = document.querySelector('main .table tbody');
      if (tbody){ tbody.innerHTML = `<tr><td colspan="9" class="text-center py-3">Loading recipients...</td></tr>`; }
        ...
        fileInput.addEventListener('change', async (e) => {
          ...
          try{
            if (typeof XLSX === 'undefined'){
              showImportModal('Import Error', '<div class="text-danger">XLSX library not loaded.</div>');
              return;
            }
            ...
            wb.SheetNames.forEach(sn => {
              ...
              const mx = XLSX.utils.sheet_to_json(ws0, { header: 1, defval: '' });
              // score: count non-empty cells
              let cells = 0;
              let headerHit = 0;
              for (let r = 0; r < Math.min(20, mx.length); r++){
                const row = mx[r] || [];
                cells += row.reduce((a,v)=>a + (String(v).trim()!==''?1:0), 0);
              ...
              if (score > chosenScore){ chosenScore = score; chosen = { name: sn, ws: ws0, matrix: mx }; }
            });
            const sheetName = chosen ? chosen.name : wb.SheetNames[0];
            const ws = chosen ? chosen.ws : wb.Sheets[sheetName];
            const matrix = chosen ? chosen.matrix : XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
            ...
            for (let i = 0; i < top; i++){
              const rowLower = (matrix[i] || []).map(c => String(c).toLowerCase().trim());
              ...
              if (hasDataBelow){ headerRowIdx = i; break; }
            }
            const headerRaw = matrix[headerRowIdx] || [];
            const header = headerRaw.map(h => String(h).trim());
            const headerLower = header.map(h => h.toLowerCase());
            const headerHasKnown = headerLower.some(c => expected.includes(c));
            let rows = [];
            if (headerHasKnown){
            ...
            if (rows.length === 0){
              const diag = `
                ...
                <div class="small mb-2">Header: <code>${header.map(h=>String(h)).join(' | ')}</code></div>
            ...
            showImportModal('Import Error', `<div class="text-danger">${escapeHtml(err.message||'Unknown error')}</div>`);
          } finally {
