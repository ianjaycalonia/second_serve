(function(){
  ...
  function badge(status){
    switch(status){
      ...
      case 'Expiring Soon': return '<span class="badge bg-warning text-dark">Expiring Soon</span>';
  ...
  async function fetchInventory(params={}){
    ...
    if (!json || json.success !== true) throw new Error(json?.error || 'Failed to load inventory');
    return json.data || { items: [], pagination: { page: 1, pages: 1, total: 0 } };
  }

  function getFilters(){
    const category = (document.getElementById('categorySelectDesktop')?.value) || (document.getElementById('categorySelectMobile')?.value) || 'All';
    const date = (document.getElementById('dateSelectDesktop')?.value) || (document.getElementById('dateSelectMobile')?.value) || 'All';
  ...
  async function loadAndRender(page=1){
    try{
      ...
      const tbody = document.querySelector('main .table tbody');
      if (tbody){
        tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger">Failed to load inventory (${escapeHtml(err.message)}). You must be logged in as Admin to view inventory.</td></tr>`;
      }
    }
  }
  ...
  function renderPagination(p){
    ...
    if (!container) return;
    const page = p.page || 1, pages = p.pages || 1;
    if (pages <= 1){ container.innerHTML = ''; return; }
    const btn = (label, target, disabled=false) => `<button class="btn btn-sm btn-outline-secondary ${disabled?'disabled':''}" data-page="${target}" ${disabled?'disabled':''}>${label}</button>`;
    const parts = [];
  ...
  function bindFilters(){
    ['categorySelectDesktop','categorySelectMobile','dateSelectDesktop','dateSelectMobile','searchInventoryInput'].forEach(id => {
      const el = document.getElementById(id);
      if (el){ el.addEventListener('change', () => loadAndRender(1)); }
    });
  ...
  function ensureSearchBox(){
    ...
    wrapper.innerHTML = '<input id="searchInventoryInput" class="form-control form-control-sm" placeholder="Search items or category">';
  ...
  async function getFoodbankRecipientId(){
    ...
    try{
      ...
      const exact = arr.find(u => String(u.organization_name||'').trim().toLowerCase() === 'foodbank (on-site)');
      if (exact && exact.user_id){ return Number(exact.user_id); }
      // Fallback: find any tagged 'onsite'
      const tagged = arr.find(u => String(u.tags||'').toLowerCase().includes('onsite'));
      if (tagged && tagged.user_id){ return Number(tagged.user_id); }
    }catch(_){ }
    return NaN;
  }
  ...
  async function ensureRunForPeriod(periodKey, note){
    ...
    try{
      if (window.AllocationsAPI && typeof window.AllocationsAPI.runByPeriod === 'function'){
    ...
    try{
      const r = await fetch(`${API_BASE_URL}/allocations/index.php?action=run_by_period&period_key=${encodeURIComponent(periodKey)}&t=${Date.now()}`, { credentials:'include', headers:{'Accept':'application/json'} });
    ...
    // Create run
    try{
      if (window.AllocationsAPI && typeof window.AllocationsAPI.createRun === 'function'){
        const cj = await window.AllocationsAPI.createRun(note || 'On-site giveaway', periodKey);
        const runId = parseInt(cj?.data?.run_id || cj?.run_id || 0, 10) || 0;
        if (runId) return runId;
      }
    }catch(_){ /* fall back */ }
    const cr = await fetch(`${API_BASE_URL}/allocations/index.php?action=create_run`, {
      method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'},
      body: JSON.stringify({ note: note || 'On-site giveaway', period_key: periodKey })
    });
    const cj = await cr.json().catch(()=>null);
    if (!cr.ok || !cj?.success) throw new Error(cj?.error || `HTTP ${cr.status}`);
  ...
  async function createOnsiteAllocation(itemName, category, quantity, note){
    ...
    let allocationId = 0;
    try{
      if (window.AllocationsAPI && typeof window.AllocationsAPI.onsiteIssue === 'function'){
        const j = await window.AllocationsAPI.onsiteIssue(itemName, category, quantity, note||'', periodKey, null);
        if (j?.success === false) throw new Error(j?.error || 'On-site issue failed');
        allocationId = Number(j?.data?.allocation_id || 0) || 0;
      }
    }catch(e){ /* fallback to fetch below */ }
    if (!allocationId){
      const resp = await fetch(`${API_BASE_URL}/allocations/index.php?action=onsite_issue`, {
        method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'},
        body: JSON.stringify({ item_name: itemName, category, quantity, note: note||'', period_key: periodKey })
      });
      const jr = await resp.json().catch(()=>null);
      if (!resp.ok || jr?.success === false) throw new Error(jr?.error || `HTTP ${resp.status}`);
      allocationId = Number(jr?.data?.allocation_id || 0) || 0;
    }
    if (!allocationId){ throw new Error('Unable to create onsite allocation.'); }
    ...
    try{
      ...
      if (body){ body.textContent = `Issued ${quantity} × ${itemName} (${category}). Allocation marked as Completed.`; }
  ...
  function parseCsv(text){
    ...
    while(i < text.length){
      ...
      if (inQuotes){
        ...
        else if (c === '\r'){ /* ignore CR, handle on next LF */ }
        else { field += c; }
      }
    }
  ...
  function bindImportModal(){
    ...
    fileInput.addEventListener('change', async (e) => {
      submitBtn.disabled = true;
      headEl.innerHTML = '';
      bodyEl.innerHTML = '<tr><td class="text-center text-muted">Parsing...</td></tr>';
      statusEl.textContent = '';
      const file = e.target.files && e.target.files[0];
      if (!file){ bodyEl.innerHTML = '<tr><td class="text-center text-muted">No file loaded.</td></tr>'; return; }
      try{
        let rows = [];
        ...
        if (!rows.length){ bodyEl.innerHTML = '<tr><td class="text-center text-muted">Empty file.</td></tr>'; return; }
        const header = rows[0].map(normalizeHeader);
        const dataRows = rows.slice(1);
        previewData = { header, rows: dataRows };
        // Render header (show original or normalized? we'll show normalized)
        headEl.innerHTML = '<tr>' + header.map(h => `<th>${h||'col'}</th>`).join('') + '</tr>';
        // Render first up to 50 rows
        const maxRows = Math.min(50, dataRows.length);
        const html = [];
        for (let r=0; r<maxRows; r++){
          const row = dataRows[r];
          html.push('<tr>' + header.map((_,i) => `<td>${escapeHtml(row[i]||'')}</td>`).join('') + '</tr>');
        }
        ...
        bodyEl.innerHTML = '<tr><td class="text-center text-danger">Failed to parse file.</td></tr>';
    ...
    submitBtn.addEventListener('click', () => {
      ...
      alert(`Import ready: ${count} rows parsed. Server endpoint not yet wired.`);
      try{
  ...
  async function init(){
    ...
    if (submit){
      submit.addEventListener('click', async () => {
        ...
        if (!itemName || !category){ if (fb) fb.textContent = 'Invalid item context.'; return; }
        if (!quantity || quantity <= 0){ if (fb) fb.textContent = 'Quantity must be at least 1.'; return; }
        const cleanup = () => {
          try{
            document.querySelectorAll('.modal-backdrop').forEach(el => { try{ el.remove(); }catch(_){ } });
