...
(function(){
  ...
  function normalizeRunId(val){
    const parsed = Number.parseInt(val, 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
  }

  function parsePeriodKeyMeta(key){
  ...
  function sortRunsDescending(list){
    ...
    arr.sort((a, b) => {
      ...
      const metaB = parsePeriodKeyMeta(b?.period_key);
      const metaA = parsePeriodKeyMeta(a?.period_key);
      if (metaB && metaA) {
        if (metaB.year !== metaA.year) return metaB.year - metaA.year;
        if (metaB.month !== metaA.month) return metaB.month - metaA.month;
        if (metaB.week !== metaA.week) return metaB.week - metaA.week;
      } else if (metaB && !metaA) {
        return -1;
      } else if (!metaB && metaA) {
        return 1;
      ...
      const bId = Number(b?.run_id ?? 0);
      const aId = Number(a?.run_id ?? 0);
      return bId - aId;
    });
    return arr;
  }
  ...
  function getFullImagePath(relativePath) {
    ...
    const val = String(relativePath).trim();
    if (val === '') return '';
    // Already absolute URL or rooted path
    if (/^https?:\/\//i.test(val) || val.startsWith('/')) {
      return val;
    }
    const normalized = val.replace(/^\/+/,'');
    if (!normalized.includes('/')) {
      if (normalized.startsWith('pickup_signatures_')) {
        return `/images/uploads/pickups/signatures/${normalized}`;
      }
      if (normalized.startsWith('pickup_photos_')) {
        return `/images/uploads/pickups/photos/${normalized}`;
      }
      if (normalized.startsWith('pickup_')) {
        return `/images/uploads/pickups/${normalized}`;
      }
    }
    return `/${normalized}`;
  }
  ...
  async function fetchRuns(){
    try {
      ...
      const j = await res.json().catch(() => null);
      runs = sortRunsDescending(Array.isArray(j?.data?.items) ? j.data.items : []);
      renderRunFilter();
    } catch(err){
  ...
  function renderRunFilter(){
    ...
    const previous = els.runFilter.value;
    ...
    const hasPrev = previous && Array.from(els.runFilter.options).some(opt => opt.value === previous);
    if (hasPrev) {
      els.runFilter.value = previous;
    } else if (forcedRunId && Array.from(els.runFilter.options).some(opt => opt.value === String(forcedRunId))) {
      els.runFilter.value = String(forcedRunId);
    }
  }
  ...
  async function fetchAllocations(){
    ...
    const runFetch = (async () => {
      ...
      let runId = normalizeRunId(currentFilters.run);
      if (!runId){
        const latest = await resolveLatestRunId();
        runId = latest;
        forcedRunId = latest;
        if (runId){
          if (!normalizeRunId(currentFilters.run)) {
            currentFilters.run = String(runId);
          }
          if (els.runFilter && !normalizeRunId(els.runFilter.value)) {
