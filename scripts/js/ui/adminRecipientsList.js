(function(){
  ...
  async function apiGetPlan(month){
    ...
    const res = await fetch(`${API_BASE_URL}/recipients/index.php?action=get_plan&month=${encodeURIComponent(month||currentMonth())}&week_start=${encodeURIComponent(ws)}`, { credentials:'include' });
  ...
  async function apiSavePlan(weeksObj, month){
    ...
    const res = await fetch(`${API_BASE_URL}/recipients/index.php?action=save_plan`, { method:'POST', headers:{'Content-Type':'application/json','Accept':'application/json'}, credentials:'include', body: JSON.stringify(body) });
