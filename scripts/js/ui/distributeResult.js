(async function(){
        ...
        if (!runId && !recipientIds.length){
          try {
            if (/^\d{4}-\d{2}-W[1-4]$/.test(periodKeyParam)){
              ...
              const runUrl = `${API_BASE_URL}/allocations/index.php?action=run_by_period&period_key=${encodeURIComponent(periodKeyParam)}&t=${Date.now()}`;
        ...
        let recMeta = new Map();
        try{
          const res = await fetch(`${API_BASE_URL}/users/index.php?action=list&role=recipient&status=approved`, { credentials:'include' });
          const j = await res.json().catch(()=>null);
        ...
        if (effectiveRunId){
          try{
            const listUrl = `${API_BASE_URL}/allocations/index.php?action=list_by_run&run_id=${encodeURIComponent(String(effectiveRunId))}&t=${Date.now()}`;
          ...
          for (const rid of recipientIds){
            try{
              const perUrl = `${API_BASE_URL}/allocations/index.php?action=list_by_recipient&recipient_id=${encodeURIComponent(rid)}&t=${Date.now()}`;
        ...
        const hasPending = byRec.some(({ items }) => Array.isArray(items) && items.some(a => String(a.status||'').toLowerCase() === 'pending'));
        if (hasPending){
          isLocked = false;
        ...
        document.getElementById('autoAllocateBtn')?.addEventListener('click', async ()=>{
          ...
          try{
            ...
            if (!periodKey && effRunId){
              try{
                let rows = [];
                if (window.AllocationsAPI && typeof window.AllocationsAPI.listRuns==='function'){
                  const jx = await window.AllocationsAPI.listRuns(100);
                  rows = Array.isArray(jx?.data?.items) ? jx.data.items : (Array.isArray(jx) ? jx : []);
                } else {
                  const r = await fetch(`${API_BASE_URL}/allocations/index.php?action=list_runs&limit=100&t=${Date.now()}`, { credentials:'include', headers:{'Accept':'application/json'} });
            ...
            let sugg = [];
            try{
              if (window.AllocationsAPI && typeof window.AllocationsAPI.previewAllocation==='function'){
                const jr = await window.AllocationsAPI.previewAllocation(periodKey, rids);
                sugg = Array.isArray(jr?.data?.allocations) ? jr.data.allocations : (Array.isArray(jr) ? jr : []);
              } else {
                const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=preview_allocation`, {
                  method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                  body: JSON.stringify({ period_key: periodKey, recipient_ids: rids })
                });
                const j = await res.json().catch(()=>null);
                if (!res.ok || !j?.success){ throw new Error(j?.error || `HTTP ${res.status}`); }
                sugg = Array.isArray(j?.data?.allocations) ? j.data.allocations : [];
              }
            } catch(e){ throw e; }
            ...
            for (const [rid, itemsMap] of aggByRid.entries()){
              ...
              try{
                if (!allocId){
                  ...
                  if (window.AllocationsAPI && typeof window.AllocationsAPI.createResult==='function'){
                    try { await window.AllocationsAPI.createResult(payload.recipient_id, payload.items, payload.run_id, payload.allocation_code, payload.notify_admin); created++; ok += itemsArr.length; }
                    catch(_){ fail += itemsArr.length; }
                  } else {
                    const rr = await fetch(`${API_BASE_URL}/allocations/index.php?action=create_result`, {
                      method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'},
                  ...
                  for (const it of itemsArr){
                    if (window.AllocationsAPI && typeof window.AllocationsAPI.addItem==='function'){
                      try { await window.AllocationsAPI.addItem(allocId, it.item_name, it.category, it.quantity); ok++; } catch(_){ fail++; }
                    } else {
                      const rr = await fetch(`${API_BASE_URL}/allocations/index.php?action=add_item`, {
                        method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'},
        ...
        function scheduleNextPoll(){
          ...
          try { if (__DR_POLL_TIMER__) clearTimeout(__DR_POLL_TIMER__); } catch(_){ }
        ...
        async function doPoll(){
          ...
          if (__DR_DIRTY__) { scheduleNextPoll(); return; }
          const effRunId = (runId || window.__DR_RESOLVED_RUN_ID__ || 0);
          if (!effRunId) { scheduleNextPoll(); return; }
          let changed = false;
          try{
            const listUrl = `${API_BASE_URL}/allocations/index.php?action=list_by_run&run_id=${encodeURIComponent(String(effRunId))}&t=${Date.now()}`;
            const res = await fetch(listUrl, { credentials:'include', headers:{'Accept':'application/json'} });
            const j = await res.json().catch(()=>null);
            if (res.ok && j?.success){
              const arr = Array.isArray(j?.data?.items) ? j.data.items : [];
              const pendingNow = arr.some(a => String(a.status||'').toLowerCase() === 'pending');
              ...
              if (pendingNow !== lastPendingState){
                ...
                if (btn){ if (wantLocked) btn.classList.add('d-none'); else btn.classList.remove('d-none'); }
              }
            }
          } catch(_){ /* ignore errors; rely on backoff */ }
          ...
          if (changed){
            ...
            pollInterval = Math.min(POLL_MAX_MS, Math.floor(pollInterval * BACKOFF_FACTOR));
            // Optional: fully stop after long inactivity; resume on visibility/user action
            if (unchangedCycles >= 12) { // ~ a few minutes depending on backoff
              pollingStopped = true;
              try { if (__DR_POLL_TIMER__) clearTimeout(__DR_POLL_TIMER__); } catch(_){ }
        ...
        container.addEventListener('input', ()=>{ /* resume if user interacts after dormancy */ if (pollingStopped) resumePollingNow(); }, { passive:true });
          ...
          if (/^W\d{1,2}$/i.test(raw)){
            ...
            try{
              ...
              if (window.AllocationsAPI && typeof window.AllocationsAPI.listRuns==='function'){
                ...
                rows = Array.isArray(jr?.data?.items) ? jr.data.items : (Array.isArray(jr) ? jr : []);
              } else {
                const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=list_runs&limit=24&t=${Date.now()}`, { credentials:'include', headers:{'Accept':'application/json'} });
                const j = await res.json().catch(()=>null);
        ...
        container.addEventListener('input', (e)=>{
          const t = e.target;
          if (t && (t.classList?.contains('dr-name') || t.classList?.contains('dr-qty'))){
            if (isLocked){
              e.preventDefault();
              return;
            }
        ...
        container.addEventListener('click', async (e)=>{
          ...
          if (itemId > 0){
            try{
              if (window.AllocationsAPI && typeof window.AllocationsAPI.deleteItem==='function'){
                await window.AllocationsAPI.deleteItem(itemId);
              } else {
                const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=delete_item`, {
                  method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'},
        ...
        document.getElementById('saveChangesBtn')?.addEventListener('click', async ()=>{
          ...
          try{
            ...
            for (const a of ops.add){
              try{
                if (window.AllocationsAPI && typeof window.AllocationsAPI.addItem==='function'){
                  await window.AllocationsAPI.addItem(a.allocation_id, a.item_name, a.category, a.quantity); ok++;
                } else {
                  const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=add_item`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'}, body: JSON.stringify(a) });
            ...
            for (const u of ops.update){
              try{
                if (window.AllocationsAPI && typeof window.AllocationsAPI.updateItem==='function'){
                  await window.AllocationsAPI.updateItem(u.item_id, u.item_name, u.quantity); ok++;
                } else {
                  const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=update_item`, { method:'PATCH', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'}, body: JSON.stringify(u) });
                  const j = await res.json().catch(()=>null); if (res.ok && j?.success) ok++; else fail++;
                }
              } catch(_){ fail++; }
            }
            for (const d of ops.del){
              try{
                if (window.AllocationsAPI && typeof window.AllocationsAPI.deleteItem==='function'){
                  await window.AllocationsAPI.deleteItem(d.item_id); ok++;
                } else {
                  const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=delete_item`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/json','Accept':'application/json'}, body: JSON.stringify(d) });
                  const j = await res.json().catch(()=>null); if (res.ok && j?.success) ok++; else fail++;
                }
              } catch(_){ fail++; }
            }
