(function(){
  ...
  async function refreshAllocations(){
    ...
    try {
      ...
      const rid = (role === 'admin' && recipientIdParam) ? Number(recipientIdParam) : null;
      ...
      if (window.AllocationsAPI && typeof window.AllocationsAPI.listByRecipient === 'function'){
        try {
          try { console.debug('[receivedItems] Using AllocationsAPI.listByRecipient', { rid }); } catch(_){ }
          items = await window.AllocationsAPI.listByRecipient(rid || undefined);
        } catch(e){
          try { console.warn('[receivedItems] AllocationsAPI.listByRecipient failed, falling back', e); } catch(_){ }
        }
      }
      if (!Array.isArray(items) || items.length === 0){
        const ridQuery = (rid ? `&recipient_id=${encodeURIComponent(String(rid))}` : '');
        const url = `${API_BASE_URL}/allocations/index.php?action=list_by_recipient${ridQuery}&t=${Date.now()}`;
        try { console.debug('[receivedItems] refreshAllocations: API URL (fallback):', url); } catch(_) {}
      ...
      if (ackBtn){
        ackBtn.addEventListener('click', async ()=>{
          try{
            ...
            let ok = false, msg = '';
            try{
              if (window.AllocationsAPI && (action==='acknowledge') && typeof window.AllocationsAPI.acknowledge==='function'){
                await window.AllocationsAPI.acknowledge(aid); ok = true;
              } else if (window.AllocationsAPI && (action==='acknowledge_admin') && typeof window.AllocationsAPI.acknowledgeByAdmin==='function'){
                await window.AllocationsAPI.acknowledgeByAdmin(aid); ok = true;
              } else {
                const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=${action}` , {
      ...
      if (scheduleBtn){
        ...
        async function doPickup(){
          try{
            ...
            let ok=false, msg='';
            try{
              if (window.AllocationsAPI && typeof window.AllocationsAPI.schedule==='function'){
                await window.AllocationsAPI.schedule(aid); ok=true;
              } else {
                const url = `${API_BASE_URL}/allocations/index.php?action=schedule`;
                const res = await fetch(url, { method: 'POST', credentials:'include', headers: { 'Content-Type':'application/json','Accept':'application/json' }, body: JSON.stringify({ allocation_id: aid }) });
                const j = await res.json().catch(()=>null);
                ok = !!(res.ok && j?.success);
                if (!ok) msg = j?.error || `HTTP ${res.status}`;
              }
            }catch(e){ ok=false; msg = e?.message || 'Failed'; }
            if (!ok){ showToast(`Failed to pick up items. ${msg}`); scheduleBtn.disabled=false; return; }
            await refreshAllocations();
          ...
          try{
            ...
            try{
              if (window.AllocationsAPI && typeof window.AllocationsAPI.complete==='function'){
                await window.AllocationsAPI.complete(aid); ok=true;
              } else {
                const res = await fetch(`${API_BASE_URL}/allocations/index.php?action=complete` , { method: 'POST', credentials:'include', headers: { 'Content-Type':'application/json','Accept':'application/json' }, body: JSON.stringify({ allocation_id: aid }) });
                const j = await res.json().catch(()=>null);
                ok = !!(res.ok && j?.success);
                if (!ok) msg = j?.error || `HTTP ${res.status}`;
              }
            }catch(e){ ok=false; msg = e?.message || 'Failed'; }
            if (!ok){ showToast(`Failed to complete. ${msg}`); completeBtn.disabled=false; return; }
            // Refresh data from server
            await refreshAllocations();
