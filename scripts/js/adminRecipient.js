...
(function () {
    ...
    async function doImport() {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/users.php?action=importRecipients`,
          {
            method: "POST",
            headers: {
  ...
  async function fetchRecipients() {
    const res = await fetch(
      `${API_BASE_URL}/users.php?action=list&role=recipient&status=approved&t=${Date.now()}`,
      {
        method: "GET",
  ...
  async function init() {
    try {
      ...
      const tbody = document.querySelector("main .table tbody");
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">Failed to load recipients (${escapeHtml(
          err.message
        )})</td></tr>`;
      }
    }
