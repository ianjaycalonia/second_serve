    ...
    document.addEventListener('click', async function(e){
      ...
      try{
        ...
        if (btnRec){
          ...
          if (!rid) { alert('Invalid recipient id'); return; }
          payload.mode = 'recipient';
          payload.recipient_id = rid;
        } else {
          payload.mode = 'onsite';
          const note = prompt('Optional note for on-site giveaway (or leave blank):') || '';
          payload.note = note;
        }

        const res = await fetch(`${API_BASE_URL}/inventory/index.php/move-out-group`, {
          method: 'POST',
